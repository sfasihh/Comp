...
LCD_DAT         EQU   PORTB     ;                     ADDED
LCD_CNTR        EQU   PTJ       ;                     ADDED
LCD_E           EQU   $80       ;                     ADDED
LCD_RS          EQU   $40       ;                     ADDED
...
BCD_BUFFER      EQU   *         ;                     ADDED
TEN_THOUS       ds.b  1         ; 10,000 digit
THOUSANDS       ds.b  1         ;  1,000 digit
HUNDREDS        ds.b  1         ;    100 digit
TENS            ds.b  1         ;     10 digit
UNITS           ds.b  1         ;      1 digit
BCD_SPARE       ds.b  10        ; Extra space for decimal point and string terminator  ADDED
...
NOT_START         CMPA  #FWD              ; Else if it’s the FORWARD state CHANGED     
                  BNE   NOT_FORWARD       ;                                         
                  JSR   FWD_ST            ; then call the FORWARD routine   CHANGED
                  JMP   DISP_EXIT         ; and exit
                  
NOT_FORWARD       CMPA  #REVER            ; Else if it’s the REVERSE state  CHANGED
                  BNE   NOT_REVERSE
                  JSR   REV_ST            ; then call the REVERSE routine   CHANGED
                  JMP   DISP_EXIT         ; and exit                  
                  
                
NOT_REVERSE       CMPA  #ALL_STP                                                  ; Else if it’s the ALL_STOP state CHANGED
                  BNE   NOT_ALL_STOP
                  JSR   ALL_STP_ST         ; then call the ALL_STOP routine  CHANGED
                  JMP   DISP_EXIT          ; and  exit
                
NOT_ALL_STOP      CMPA  #FWD_TRN           ; Else if it’s the FORWARD_TURN state  CHANGED
                  BNE   NOT_FWD_TRN                                              ;CHANGED
                  JSR   FWD_TRN_ST         ; then call the FORWARD_TURN routine   CHANGED
                  JMP   DISP_EXIT          ; and  exit                
                  

NOT_FWD_TRN       CMPA  #REV_TRN          ; Else if it’s the REV_TRN state        CHANGED
                  BNE   NOT_REV_TRN                                              ;CHANGED
                  JSR   REV_TRN_ST        ; then call REV_TRN_ST routine          CHANGED
                  BRA   DISP_EXIT         ; and exit
...
;*******************************************************************
START_ST          BRCLR PORTAD0,$04,NO_FWD   ; If /FWD_BUMP      CHANGED
                  JSR   INIT_FWD             ; Initialize the FORWARD state
                  MOVB  #FWD,CRNT_STATE      ;                                    CHANGED
                  BRA   START_EXIT           ;
...
FWD_ST          BRSET PORTAD0,$04,NO_FWD_BUMP       ; If FWD_BUMP then                           
                ...
                MOVB  #REVER,CRNT_STATE               ; set the state to REVERSE   CHANGED
                JMP   FWD_EXIT                      ; and return
                
NO_FWD_BUMP     BRSET PORTAD0,$08,NO_REAR_BUMP      ; If REAR_BUMP, then we should stop
                JSR   INIT_ALL_STP                  ; so initialize the ALL_STOP state     CHANGED
                MOVB  #ALL_STP,CRNT_STATE           ; and change state to ALL_STOP         CHANGED
                JMP   FWD_EXIT                      ; and return
                
NO_REAR_BUMP    LDAA  TOF_COUNTER                   ; If Tc>Tfwd then                      CHANGED
                CMPA  T_FWD                         ; the robot should make a turn         CHANGED
                BNE   NO_FWD_TRN                    ; so                                   CHANGED
                JSR   INIT_FWD_TRN                  ; initialize the FORWARD_TURN state    CHANGED
                MOVB  #FWD_TRN,CRNT_STATE           ; and go to that state                 CHANGED
                JMP   FWD_EXIT                                                
...
UPDT_DISPL      MOVB  #$90,ATDCTL5          ; R-just., uns., sing. conv., mult., ch=0, start
                ...
                ADDD  #600                  ; AccD = 1st result x 39 + 600  (for poteitomenter)
